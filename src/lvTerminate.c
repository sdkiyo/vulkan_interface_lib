...
int lvTerminate(lvData *const lvdata)
{
	...
	PFN_vkDestroySemaphore pfn_vkDestroySemaphore = (PFN_vkDestroySemaphore)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroySemaphore");
	PFN_vkDestroyFence pfn_vkDestroyFence = (PFN_vkDestroyFence)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroyFence");
	...
	PFN_vkDestroyCommandPool pfn_vkDestroyCommandPool = (PFN_vkDestroyCommandPool)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroyCommandPool");
	pfn_vkDestroyCommandPool(lvdata->device, lvdata->commandPool, nullptr);
	printf(YELLOW "- vkDestroyCommandPool()" RESET_COLOR "\n");

	PFN_vkDestroyFramebuffer pfn_vkDestroyFramebuffer = (PFN_vkDestroyFramebuffer)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroyFramebuffer");
	for (uint32_t i = 0; i < IMAGE_COUNT; i++)
	{
	...
	PFN_vkDestroyPipeline pfn_vkDestroyPipeline = (PFN_vkDestroyPipeline)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroyPipeline");
	pfn_vkDestroyPipeline(lvdata->device, lvdata->graphicsPipeline, nullptr);
	printf(YELLOW "- vkDestroyPipeline()" RESET_COLOR "\n");

	PFN_vkDestroyPipelineLayout pfn_vkDestroyPipelineLayout = (PFN_vkDestroyPipelineLayout)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroyPipelineLayout");
	pfn_vkDestroyPipelineLayout(lvdata->device, lvdata->pipelineLayout, nullptr);
	printf(YELLOW "- vkDestroyPipelineLayout()" RESET_COLOR "\n");

	PFN_vkDestroyRenderPass pfn_vkDestroyRenderPass = (PFN_vkDestroyRenderPass)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroyRenderPass");
	pfn_vkDestroyRenderPass(lvdata->device, lvdata->renderPass, nullptr);
	printf(YELLOW "- vkDestroyRenderPass()" RESET_COLOR "\n");

	PFN_vkDestroyImageView pfn_vkDestroyImageView = (PFN_vkDestroyImageView)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroyImageView");
	...
	PFN_vkDestroySwapchainKHR pfn_vkDestroySwapchainKHR = (PFN_vkDestroySwapchainKHR)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroySwapchainKHR");
	pfn_vkDestroySwapchainKHR(lvdata->device, lvdata->swapchain, nullptr);
	printf(YELLOW "- vkDestroySwapchainKHR()" RESET_COLOR "\n");

	PFN_vkDestroyDevice pfn_vkDestroyDevice = (PFN_vkDestroyDevice)lvdata->pfn_vkGetDeviceProcAddr(lvdata->device, "vkDestroyDevice");
	pfn_vkDestroyDevice(lvdata->device, nullptr);
	printf(YELLOW "- vkDestroyDevice()" RESET_COLOR "\n");

	PFN_vkDestroySurfaceKHR pfn_vkDestroySurfaceKHR = (PFN_vkDestroySurfaceKHR)lvdata->pfn_vkGetInstanceProcAddr(lvdata->instance, "vkDestroySurfaceKHR");
